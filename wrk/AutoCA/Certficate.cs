		...
		*/
		...
		public bool CreateCA(OrgProfile pOrgProfile, string pCommonName, Certificate pCACertificate)
		{
			...
			if (pCACertificate == null)
			{
				...
				m_pCertificate = CertificateProvider.CreateSelfCertificate(pRequest, iLifeDays);
			...
			//　証明書記載情報の主要なものをキャッシュ
			FetchProperties(m_pCertificate, pKeys, pCommonName, true);
		...
		public bool CreateForServer(OrgProfile pOrgProfile, string pCommonName, string pFQDN, Certificate pCACertificate)
		{
			...
			//　証明書記載情報の主要なものをキャッシュ
			FetchProperties(m_pCertificate, pKeys, pCommonName, false);
		...
		private void FetchProperties(X509Certificate2 pCertificate, ECDsaCng pKeys, string pCommonName, bool IsCA)
		{
			SequenceNumber = 0;
